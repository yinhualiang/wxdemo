                ...
                <div v-if="isListView" class="box2">
                    <div v-for="item in data" :key="item.id">
                        <div class="card facebook-card animated fadeInUp" @click="showDetail(item)">
                            <div v-if="!util.isNotEmpty(item.endTime) && item.category == 6" class="ribbon-wrapper-green">
                            ...
                            <div class="card-header no-border">
                               ...
                               <div class="facebook-date">
                                    ...
                                    <span class=" c999 title-item" style="font-size: 9pt;width:50%;float: left;">当前环节:{{item.actName}}</span>
...
<script>
import { Timeline, TimelineItem, Scroller, LoadMore, XCircle, XInput, Search } from 'vux'
...
export default {
    ...
    this.util.init(this);
        ...
        var category = this.util.getUrlParam("type");
        if (this.util.isNotEmpty(category)) {
            this.category = category;
        }
        var type = this.util.getUrlParam("type");
        if (this.util.isNotEmpty(type)) {
            this.noMore = false;
            this.isEnd = false;
            this.type = type;
            _this.data = [];
            if (this.type == 'finished') {
                $(document).attr("title", "我的已办");
                this.pageType = 'approval';
            } else {
                $(document).attr("title", "我的待办");
                this.pageType = 'unApproval';
            }
        }
    ...
    methods: {
        ...
        onScrollBottom() {
            ...
            if (this.util.isNotEmpty(this.type)) {
            ...
            this.util.post('/wx/workflowAction!findMyApproval.action?pageSize=' + _this.pageSize, param, function (res) {
            //this.util.post('/open/workflowAction!findMyApproval.action?pageSize=' + _this.pageSize, param, function (res) {
            //this.util.post('/open/openWorkflowAction!findMyApproval.action?pageSize=' + _this.pageSize, param, function (res) {
                //_this.util.unmask();
                if ((_this.util.isNotEmpty(_this.search) || _this.advancedQuery.enable) && type == undefined) {
                    _this.util.unmask();
                }
        ...
        showDetail(item) {
            ...
            console.info(item);
            console.debug("item.formKey====="+item.formKey);
            ...
            if (this.util.isNotEmpty(item.formKey)) {
                if (this.type == "unFinished") {
                    ...
                    if (item.formKey == 'apply_goods') {
                        this.$router.push({ path: '/apply/applyGoods', query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id, isReadOnly: true } });
                    } else if (item.formKey == 'expense') {
                        this.$router.push({ path: '/movebx', query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id, isReadOnly: true } });
                    } else if (item.formKey == 'leave') {
                        this.$router.push({ path: '/addLeave', query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id, isReadOnly: true } });
                    }else if (item.formKey == 'apply_purchase') {
                        this.$router.push({ path: '/apply/applyPurchase', query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id, isReadOnly: true } });
                    }else if (item.formKey == 'apply_borrowing') {
                        this.$router.push({ path: '/apply/applyBorrowing', query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id, isReadOnly: true } });
                    }else if (item.formKey == 'apply_payment') {
                        this.$router.push({ path: '/apply/applyPayment', query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id, isReadOnly: true } });
                    }else if (item.formKey == 'apply_workover') {
                        this.$router.push({ path: '/apply/applyWorkover', query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id, isReadOnly: true } });
                    }else if (item.formKey == 'apply_common') {
                        this.$router.push({ path: '/apply/applyCommon', query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id, _taskType: item.category, _currentActId: item.actId, _taskStatus: status, isReadOnly: true } });
                    }
                }
            }else{
                this.$router.push({ path: '/processForm', query: { id: item.businessKey,_procDefId: item.procDefId,_procInstId:item.procInsId,_taskId:item.id,_currentActId:item.taskDefKey,_startUserId:item.startUserId,_assignee:item.assignee,_taskType:0,_pageType:this.pageType,_procDefName: encodeURI(item.procDefName),_actName: encodeURI(item.actName),_subject: this.util.isNotEmpty(item.subject)?encodeURI(item.subject):''}});
            }
        ...
        deleteGou(item) {
            ...
            this.$router.push({ name: 'proj/workLog/info', query: { isReadOnly: false } });
        }, query() {
