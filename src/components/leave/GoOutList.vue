  ...
  <div class="choose">
      <div class="row" style="margin:0;background:#fff;border-bottom:1px solid #EFEFF4;height:44px;overflow:hidden" >
        <div v-if="!swichFlag" class="col-xs-7" style="padding:0;line-height: 44px;width:199px;text-align: left;overflow:hidden">
            <datetime v-model="dateStart" @on-confirm="startDay" placeholder="请选择日期" style="padding:0;float:left;margin-left:3px;"></datetime>
            <span style="padding:0;float:left;margin:0 4px;">到</span>
            <datetime v-model="dateEnd" @on-confirm="endDay" placeholder="请选择日期" style="padding:0;float:left;"></datetime>
        </div>
      ...
      <div id="scroller">
          <scroller height="-45" :use-pulldown="true" :pulldown-config="pulldown" :use-pullup="true" :pullup-config="pullup" lock-x @on-pulldown-loading="loadTop" @on-pullup-loading="onScrollBottom" ref="scrollerBottom">
            <div class="box2"> 
                <div v-for="(item,index) in dataLists" :key="index" class="jr-bill-out" @click="editOutWork(item)">
                    ...
                    <div class="jr-bill-content">
                      <p  class="jr-bill-name">{{item.name}}</p>
                      <span class="jr-bill-startday">外出天数：{{item.ccts}}</span>
                    </div>
                    <div class="jr-bill-content1">
                      <span class="jr-bill-allday">费用预计：{{item.yjfy}}</span>
                      <span class="jr-bill-startday">日期：{{item.fsrq}}</span>
                    </div>
                </div>
...
export default {
  ...
  activated(){
      this.util.init(this);
      if(this.temp!=null&&typeof this.temp=="string"&&this.temp.indexOf("loadGoOutList")>=0){
        this.loadTop();
  ...
  methods:{
    ...
    editOutWork(item){
        this.$router.push({name:'leave/addGoOut',query:{id:item.id}});
    },
    swichSearch(){
        this.swichFlag=true;
    },
    add(){
      this.$router.push({name:'leave/addGoOut'});
    },
    cancelClick(){
