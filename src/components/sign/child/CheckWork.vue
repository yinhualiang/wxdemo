		...
		geocoder.getAddress(lnglat,function(status,result){
			if(status=='complete'){
				...
				if(!_this.hadSign){   //签到
					...
					var mainData = {SIGN_OUT_LOCATION_:location,SIGN_OUT_ADDRESS_:address,SIGN_OUT_REMARK_:_this.remark,sign_out_status_:_this.leaveEarly,sign_out_time_:new Date().format("yyyy-MM-dd hh:mm")};
					params = {userUid:_this.userUid,filesData:fileDataJson,deleteFilesData: _this.attachmentModel.deleteFilesList.join(","),
							mainFormData:JSON.stringify(mainData),sqlTableName:"T_CHECK_RECORD",
