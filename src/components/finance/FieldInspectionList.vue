            ...
            <div class="box2">
                <div v-for="(item,index) in dataLists" :key="index" class="jr-bill-out" @click="editOutWork(item)" >
                    ...
                    <div class="jr-bill-content">
                        <p  class="jr-bill-name">检查人：{{item.AddName_CN}}</p>
                    </div>
                    <div class="jr-bill-content1">
                        <span class="jr-bill-startday" style="float:left">检查日期：{{item.DocCreated}}</span>
                    </div>
...
//    })
  ...
  methods:{
    ...
    editOutWork(item){
        this.$router.push({name:'finance/FieldInspectionItem',query:{id:item.Id}});
    },
    swichSearch(){
        this.swichFlag=true;
    },
    add(){
      this.$router.push({name:'finance/FieldInspectionItem'});
    },
    cancelClick(){
