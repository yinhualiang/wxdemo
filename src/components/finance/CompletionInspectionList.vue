            ...
            <div class="box2"> 
                <div v-for="(item,index) in dataLists" :key="index" class="jr-bill-out" @click="editOutWork(item)" >
                    ...
                    <div class="jr-bill-content1">
                      <span class="jr-bill-startday">验收区域{{item.qy}}</span>
                    </div>
...
export default {
  ...
  methods:{
    ...
    editOutWork(item){
        this.$router.push({name:'finance/CompletionInspectionItem',query:{id:item.Id}});
    },
    swichSearch(){
        this.swichFlag=true;
    },
    add(){
      this.$router.push({name:'finance/CompletionInspectionItem'});
    },
    cancelClick(){
