            ...
            <div class="box2"> 
                <div v-for="(item,index) in dataLists" :key="index" class="jr-bill-out" @click="editOutWork(item)" >
                    ...
                    <div class="jr-bill-content1">
                      <span class="jr-bill-startday">开工日期：{{item.kgrq}}</span>
                    </div>
...
export default {
  ...
  methods:{
    ...
    editOutWork(item){
        this.$router.push({name:'CommencementReportitem',query:{id:item.id}});
    },
    swichSearch(){
        this.swichFlag=true;
    },
    add(){
      this.$router.push({name:'CommencementReportitem'});
    },
    cancelClick(){
