            ...
            <div class="box2"> 
                <div v-for="(item,index) in dataLists" :key="index" class="jr-bill-out" @click="editOutWork(item)" v-if="item.flag">
                    ...
                    <div class="jr-bill-content1">
                        ...
                        <span class="jr-bill-startday" style="color:#FF9E16">备用金额：{{item.totalamount}}</span>
                    </div>
...
export default {
  ...
  methods:{
    ...
    editOutWork(item){
        this.$router.push({name:'finance/CompletionProblemItem',query:{id:item.id}});
    },
    swichSearch(){
        this.swichFlag=true;
    },
    add(){
      this.$router.push({name:'finance/CompletionProblemItem'});
    },
    cancelClick(){
