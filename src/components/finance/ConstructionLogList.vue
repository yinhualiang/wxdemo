            ...
            <div class="box2"> 
                <div v-for="(item,index) in dataLists" :key="index" class="jr-bill-out" @click="editOutWork(item)" >
                    <div class="jr-bill-title">
                      <span class="jr-bill-code">单号：{{item.sn}}</span>
                    ...
                    <div class="jr-bill-content">
                      <p  class="jr-bill-name">操作人：{{item.czy}}</p>
                    </div>
                    <div class="jr-bill-content1">
                      <span class="jr-bill-startday">项目名称：{{item.xmmc}}</span>
                    </div>
...
export default {
  ...
  methods:{
    ...
    loadData(type, creatTime) {
        ...
        this.util.post('/admin/appAgentAction!runAgent.action?agentId=Agent_WeChat_MobileList&appId=wechat', param, function(res) {
            _this.noMore = true;
    ...
    editOutWork(item){
        ...
        this.$router.push({name:'finance/ConstructionLogItem',query:{id:item.Id}});
    },
    swichSearch(){
        this.swichFlag=true;
    },
    add(){
      this.$router.push({name:'finance/ConstructionLogItem'});
    },
    cancelClick(){
