...
export default {
	...
	props: ['model'],
	activated() {
		//console.info("attachment--------activated--------");
		console.info(this.model.fileList);
		//this.model.fileList = [{id:1,src:'111',fileId:'12'}];
		return;
	...
	methods: {
		loadData(id) {
			...
			if (this.util.isNotEmpty(id)) {
				...
				//console.info(this.model.xtype+"----111--------------"+id);
				this.util.post("/wx/weChatAction!getFiles.action", params, function(res) {
					...
					if (res.data != undefined && res.data.length > 0) {
						...
						console.info(_this.model.fileList);
					}
				});
			}
		},
