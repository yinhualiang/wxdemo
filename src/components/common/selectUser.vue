    ...
    <section :style="sectionPaddingTop">
        <div id="fixedHeight">
            ...
            <ul class="tabImgInUl" v-if="arr.length!=0">
                <li v-for="vt in arr" @click="deleteListArr(vt.id)" class="tabImgInLi" :data-id="vt.id">
                    <div class="tabImg">
        ...
        <swiper height="100%" width="100%" :threshold="100" :show-dots="false" v-model="weatherIndex" id="yuandibaozha">
            ...
            <swiper-item>
                <!--:VObject="VObject"-->
                <SelectUserItems @ee="creatNear" :radioSelectFlags="radioSelectFlag" v-bind:model='model' :orgUsersObj='orgUsersObj' @messageOut="messageIn" v-bind:level=1 v-for='model in list' :key="model.id">
                </selectUserItems>
            </swiper-item>
        </swiper>
    ...
    methods: {
        ...
        address(num) {
            ...
            for (var i = 0; i < this.searchAddressList.length; i++) {
                //var grapheme =this.searchAddressList[i].enCode[0].toUpperCase();
