            ...
            <div class="box2"> 
                <div v-for="(item,index) in dataLists" :key="index" class="jr-bill-out" @click="editOutWork(item)" v-if="item.flag">
                    ...
                    <div class="jr-bill-content">
                        <p  class="jr-bill-name">酒店名称：{{item.xyjd}}</p>
                    </div>
                    <div class="jr-bill-content1">
                        <span class="jr-bill-startday" >申请日期：{{item.fsrq}}</span>
                        <span class="jr-bill-startday">申请人：{{item.sqr}}</span>
                    </div>
...
export default {
  ...
  methods:{
    ...
    editOutWork(item){
        this.$router.push({name:'finance/FieldProblemsItem',query:{id:item.id}});
    },
    swichSearch(){
        this.swichFlag=true;
    },
    add(){
      this.$router.push({name:'finance/FieldProblemsItem'});
    },
    cancelClick(){
