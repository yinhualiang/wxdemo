<template>
	<div style="overflow: auto;height: 100%;background: #eee;text-align: left;">
		...
		<search  v-model="sousuo"  placeholder='搜索' cancel-text='取消'></search>
		
		<div v-for="item in items"  style="background-color:#fff;padding:3% 3%;">
            <div class='container-flex'>
                <div style="width:50%">
                    <p><span>店铺名称:</span><span>{{item.dpmc}}</span></p>
                    <p><span>维修源:</span><span>{{item.wxy}}</span></p>
                    <p><span>申请人:</span><span>{{item.sqr}}</span></p>
                </div>
                <div style="width:50%">
                   <span class='pai'>派工</span>
                </div>
            </div>
        </div>
...
export default {
	...
	components: {
		// 名字
		Attachment,SelectUser,Datetime,selectQuarters,VoiceRecognition,SelectProj,ProcessForm,Search
	},
	computed: {
		...mapState({
