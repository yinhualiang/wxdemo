            ...
            <div class="box2"> 
                <div v-for="(item,index) in dataLists" :key="index" class="jr-bill-out" @click="editOutWork(item)" v-if="item.flag">
                    ...
                    <div class="jr-bill-content">
                        <span class="jr-bill-startday"  style="max-width:160px;">部门名称：{{item.bmmc}}</span>
                        <p  class="jr-bill-name">项目名称：{{item.xmmc}}</p>
                    </div>
                    <div class="jr-bill-content1">
                        <span class="jr-bill-startday">操作员：{{item.czy}}</span>
...
export default {
  ...
  methods:{
    ...
    loadData(type, creatTime) {
        ...
        this.util.post('/admin/appAgentAction!runAgent.action?agentId=Agent_WeChat_bx&appId=wechat', param, function(res) {
            _this.noMore = true;
            if (_this.util.isNotEmpty(_this.search) && type == 'downLoad') {
                _this.util.unmask();
            }
            ...
            console.debug(_this.dataLists);
        });
    },
    editOutWork(item){
        this.$router.push({name:'finance/MaintenanceRecordsItem',query:{id:item.id}});
    },
    swichSearch(){
        this.swichFlag=true;
    },
    add(){
      this.$router.push({name:'finance/MaintenanceRecordsItem'});
    },
    cancelClick(){
