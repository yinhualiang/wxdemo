<template>
	<div id="mainForm" style="padding-bottom: 0px;width: 100%;padding-bottom: 90px;" ref="mainForm">
		<div style="height: 100%;overflow:auto;width: 100%;">
			<div v-if="pageType != ''" style="">
				<div class="price" style="text-align:left;">
					<span class="col-xs-3" style="padding-right: 0px;padding-left: 10px;">流程名称:</span>
					<span>{{processModel.subject == undefined?'':(processModel.subject.length>18?processModel.subject.substr(0,18)+'...':processModel.subject)}}</span>
				</div>
				<div v-if="pageType != '' && pageType != 'apply'" class="price" style="text-align:left;">
					<span class="col-xs-3" style="padding-right: 0px;padding-left: 10px;">当前节点:</span>
				...
				<div class="price" style="text-align:left;">
					...
					<span class="col-xs-6">
						...
						<a href="javascript:void(0)" @click="redirect(1);" class="btn btn-info">查看单据数据</a>
					</span>
					<span class="col-xs-6" style=" float: right; text-align: right;padding-right: 15px;">
						<!-- <el-button size="small" @click="lookWorkflowPng();" type="primary">查看流程图</el-button> -->
						<a href="javascript:void(0)" @click="redirect(2);" class="btn btn-info">查看流程图</a>
					</span>
			...
			<div v-if="isWorkflow">
				<div v-for="(activity,index) in activities" :key="activity.id">
					<div v-if="!activity.show" class="middle" :key="index">
						<div class="pop" style="background-color: #eeefef;border-bottom: 1px #d5d5d8 solid;">
							<span class="col-xs-3" style="width: 75%;text-align: left;padding-left: 10px;">

								<input v-if="activity.selectType=='disabled'" @click.stop="inputClick(activity,index)" :name="activity.selectType" :type="activity.selectType=='radio'?'radio':'checkbox'" disabled='disabled' :checked="activity.checked" :id="activity.id">
								<input v-if="activity.selectType!='disabled'" @click.stop="inputClick(activity,index)" :name="activity.selectType" :type="activity.selectType=='radio'?'radio':'checkbox'" :checked="activity.checked" :id="activity.id"> &nbsp;{{activity.name}}&nbsp;&nbsp;{{activity.transition == activity.name?'':activity.transition}}

							</span>

							<div class="daright col-xs-9" style="float: right;width: 25%;text-align: right; " @click.stop="show(activity,index)">
					...
					<div v-if="activity.show" :key="index" class="middle">
						<div class="pop" style="background-color: #eeefef;">
							<span class="col-xs-4" style="width: 75%;text-align: left;padding-left: 10px;">
								<input v-if="activity.selectType=='disabled'" @click.stop="inputClick(activity,index)" :name="activity.selectType" :type="activity.selectType=='radio'?'radio':'checkbox'" disabled='disabled' :checked="activity.checked" :id="activity.id">
								<input v-if="activity.selectType!='disabled'" @click.stop="inputClick(activity,index)" :name="activity.selectType" :type="activity.selectType=='radio'?'radio':'checkbox'" :checked="activity.checked" :id="activity.id"> &nbsp;{{activity.name}}&nbsp;&nbsp;{{activity.transition == activity.name?'':activity.transition}}
							</span>
							<div class="daright col-xs-8" style="width: 25%; float: right;text-align: right; " @click.stop="hide(activity)">
								<i class="glyphicon glyphicon-chevron-up"></i>
							</div>
						</div>
						<div class="load" style="text-align:left;border-top: 0px solid #eee;min-height:80px;width:100%;">
								...
								<div class="load">
									<ul id="imgsLoad" style="margin-bottom:0px;margin-left: 5px;">
										<li style="margin-top: 2px;height:85px;overflow-y:auto;overflow-x:hidden;width:100%;">
											<ul class="clearfix">
												<li style="margin:5px 5px 0px 5px;position:relative;text-align:left;float:left;" v-for="procUser in activity.newProcUsers" :key="procUser.id">
													<div>
														<div>
														...
														<div style="text-align:center; font-size: 10px;line-height: 32px;height: 32px;">
															<span>{{procUser.procUserName.length>3?procUser.procUserName.substring(0,3):procUser.procUserName}}</span>
														</div>
													</div>
													<i @click="removeProcUser(procUser,$event,activity)" v-if="showProcUserDelete" class="changeDelete" style="position:absolute;top:-5px;left:-5px;background:#f00;width:20px;height:20px;border-radius:50%;color:#fff">&nbsp;X</i>
												</li>
												<li v-if="activity.ifSelUser" style="margin:5px 0px 5px 3px;position:relative;text-align:left;float:left;">
													<div><img style="width:50px;height:50px;" @click="addProcUser(activity)" src="../../assets/add_image.png"></div>
												</li>
												<li v-if="activity.ifSelUser" style="margin:5px 5px 5px 5px;position:relative;text-align:left;float:left;">
				...
				<div style="text-align:left;border-top: 0px solid #eee;width:100%;">
					<div v-if="(isWorkflow && ifCopy) ">
						<div class="pop" style="background-color: #eeefef;">
							<span class="col-xs-4" style="width: 75%;padding-left: 10px;">选择抄送人</span>
						</div>
						<div class="load">
							<ul id="imgsLoad" style="margin-bottom:0px;margin-left: 5px;">
								<li style="margin-top: 2px;height:85px;overflow-y:auto;overflow-x:hidden;width:100%;">
									<ul class="clearfix">
										<li style="margin:5px 5px 0px 5px;position:relative;text-align:left;float:left;" v-for="user in copyUserModel.users" :key="user.id">
											...
											<i @click="removeCopyUser(user,$event)" v-if="showCopyUserDelete" class="changeDelete" style="position:absolute;top:-5px;left:-5px;background:#f00;width:20px;height:20px;border-radius:50%;color:#fff">&nbsp;X</i>
										</li>
			...
			<actionsheet v-model="workflowSelShow" :menus="workflows" @on-click-menu="workflowSel"></actionsheet>
			<p style="background-color:rgb(239, 239, 239);   margin: 0px;padding-bottom: 15px;    position: fixed;width: 100%;bottom: 0px;">
				<button v-if=" ifHashModel && !model.ifHashWorkflow && showBtn.cancle" style="width:48%;height: 45px;line-height: 45px;border: 1px solid #ccc;" @click="onClick(1)" class="weui-btn weui-btn_mini weui-btn_default">取消</button>
				<button v-if=" ifHashModel &&!model.ifHashWorkflow && showBtn.cancle" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(2)" class="weui-btn weui-btn_mini weui-btn_primary">保存</button>
				<button v-if="ifHashModel && model.ifHashWorkflow && (showBtn.start || showBtn.submit)" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(2)" class="weui-btn weui-btn_mini weui-btn_default">保存草稿</button>
				<button v-if="ifHashModel && model.ifHashWorkflow && showBtn.start" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(3)" class="weui-btn weui-btn_mini weui-btn_primary">提交办理</button>
				<button v-if="ifHashModel && model.ifHashWorkflow && showBtn.submit" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(4)" class="weui-btn weui-btn_mini weui-btn_primary">提交办理</button>
				<button v-if=" !ifHashModel  && showBtn.cancle" style="width:48%;height: 45px;line-height: 45px;border: 1px solid #ccc;" @click="onClick(1)" class="weui-btn weui-btn_mini weui-btn_default">取消</button>
				<button v-if=" showBtn.reject" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(5)" class="weui-btn weui-btn_mini weui-btn_warn">不同意</button>
				<button v-if=" !ifHashModel &&showBtn.submit" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(4)" class="weui-btn weui-btn_mini weui-btn_primary">提交</button>
				<button v-if=" showBtn.read" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(6)" class="weui-btn weui-btn_mini weui-btn_primary">已阅</button>
				<button v-if=" showBtn.revoke" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(7)" class="weui-btn weui-btn_mini weui-btn_default">撤回</button>
				<button v-if=" showBtn.reminder" style="width:48%;height: 45px;line-height: 45px;" @click="onClick(8)" class="weui-btn weui-btn_mini weui-btn_primary">催办</button>

			</p>
			<div v-transfer-dom>
				<popup v-model="processPopupShow" position="right" style="width: 100%;padding-bottom:0px;">
					<div style="width:100%;    height: 100%;">
						<FormDetailData :model="formDetailData" ref="formDetailData" style="margin-bottom: 44px;"></FormDetailData>
						<a href="javascript:;" @click="processPopupShow = false;$refs.formDetailData.hide();" class="weui-btn weui-btn_warn" style="position: fixed;bottom: 0px;width: 100%;">关闭</a>
					</div>
...
export default {
	...
	activated() {
		...
		if (this.model == undefined || this.model == null) {
			...
			this.ifHashModel = false;
		} else {
			this.ifHashModel = true;
		...
		if (this.util.isNotEmpty(this.pageType)) {
			if (!this.util.flag && this.pageType == 'unRead') {
				...
				_this.util.post('/wx/workflowAction!getProcessFormData.action', param, function(res) {
				...
				this.processModel.subject = this.util.isNotEmpty(subject) ? decodeURIComponent(subject) : decodeURIComponent(this.processModel.procDefName);
				this.processModel.currentActName = this.util.isNotEmpty(actName) ? decodeURIComponent(actName) : '';
		...
		if (!this.util.isNotEmpty(this.processModel.procInsId)) {
			//console.info("-----2------");
			if (this.ifHashModel && this.model.ifHashWorkflow && !this.processModel.isReadOnly) {
				//console.info("-----3------");
				this.showBtn.start = true;
			} else if (this.ifHashModel && !this.model.ifHashWorkflow && !this.processModel.isReadOnly) {
		...
		//console.info(this.processModel);
	...
	components: {
		// 名字
		XTextarea, Actionsheet, VoiceRecognition, FormDetailData, TransferDom, Popup, SelectUser, ProcessMonitor, XButton
	},
	...
	methods: {
		...
		initProcess() {
			...
			if (this.util.isNotEmpty(this.processModel.procDefId)) {
				...
				if (this.ifHashModel) {
					_this.processModel.mainFormData = _this.getMainFormData();
				} else {
					_this.processModel.mainFormData = null;
				}

				var param = { procInsId: this.processModel.procInsId, procDefId: this.processModel.procDefId, taskId: this.processModel.taskId };
				//如果是待办办
				if (this.pageType == 'unApproval') {
					var status = '';
				...
				_this.util.post('/wx/workflowAction!getProcessFormData.action', param, function(res) {
					...
					if (res != undefined && res != null && res.status == 200) {
						...
						if ("undefined" != typeof data.mainFormDataMap && data.mainFormDataMap != null && _this.processModel.mainFormData == null) {
						...
						if ("undefined" != typeof data.sessionData) {
							_this.util.extend(_this.processModel.mainFormData, data.sessionData);
						}
						if (_this.processModel.mainFormData != null && _this.util.isNotEmpty(_this.processModel.mainFormData['Subject'])) {
							_this.processModel.subject = _this.processModel.mainFormData['Subject'];
						} else {
		...
		onClick(type) {
			...
			var params = null;
			//console.info("=======0000======"+this.model.parent);
			if ((type == 2 || type == 3 || type == 4) && this.ifHashModel && this.model.parent != undefined) {
			...
			//console.info("=======11======");
			if (type == 3) {
				if (_this.util.isNotEmpty(this.model.sqlTableName) || _this.util.isNotEmpty(this.model.formId)) {
					_this.util.failueToast("该表单没有绑定流程！");
					return false;
				}
				var param = { sqlTableName: this.model.parent.sqlTableName, formId: this.model.parent.formId, businessKey: this.processModel.businessKey };
				_this.util.mask("正在加载流程信息..");
				_this.util.post('/workflow/workflowAction!isHasWorkflow.action', param, function(res) {
					_this.util.unmask();
			...
			if (type != 2) {
				...
				if (_this.ifComment) {
					if (!t_thishis.util.isNotEmpty(_this.voiceRecognition.content)) {
				...
				for (var i = 0; i < this.activities.length; i++) {
					...
					if (activity.checked) {
						...
						if (activity.taskType != 'endEvent') {
							if (activity.newProcUsers == undefined || activity.newProcUsers.length == 0) {
								_this.util.failueToast("环节" + activity.name + "未设置处理人！");
								return false;
							}
			...
			// console.info(params.condition.length+"==="+params.condition);
			// console.info(params);
			//return;
		...
		getProcUserByFormField(select, fieldId) {
			...
			if (userIds != undefined && userIds != "" && userIds != null) {
				_this.util.post('/admin/orgUserAction!getUsersByUserId.action', { userId: userIds, type: type }, function(res) {
					if (res.length > 0) {
						for (var i = 0; i < res.length; i++) {
							newProcUsers.push({ id: res[i]['id'], procUserId: res[i]['userId'], procUserName: res[i]['userName'] });
						}
					}
				});
			}
			...
			var value = condition.substring(condition.indexOf(operate) + operate.length).trim();
			var obj =
				{
					name: name,
					value: value
			...
			if (activity.selectType == 'checkbox') {
				...
				for (var i = 0; i < this.activities.length; i++) {
					if (this.activities[i].id != activity.id && this.activities[i].selectType == 'radio') {
		...
		delProcUser() {
			...
			console.debug("this.processModel.pageType ==="+this.processModel.pageType );
			...
			if (type == 1) {
				if (this.processModel.pageType == 'unApproval') {
					detailDataUrl = this.util.getBaseUrl()
						...
						+ '&status=' + this.processModel.status + '&taskType=' + this.processModel.taskType
						+ '&company_id=' + this.sessionUtil.getCompanyId()
