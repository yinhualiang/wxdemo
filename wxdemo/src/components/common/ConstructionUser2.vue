        ...
        methods:{
            messageIn(data){
               ...
               var id=this.$store.state.myUserList[0].id;
                var param ={operateType:'jgysjc',xmid:id}
                ...
                let params={operateType:'qyys',xmid:id,xxjb:this.caModel.xinxi.xxjb}
                console.log(params)
                this.util.post('/admin/appAgentAction!runAgent.action?agentId=Agent_WeChat_Mobile &type=getInfo&sqlWhere=&node=root', params, (res)=> {	
                    this.util.unmask();
                   console.log(6666)
                   console.log(res)
					this.caModel.shop=res.data;
